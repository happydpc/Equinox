...
sub htmlifypods_target {
    ...
    my $htmlify = <<END;
htmlifypods : $dependencies
END

   $htmlify .= "\t\$(NOECHO) \$(POD2HTML)";
...
sub init_INST {
    ...
    $self->{INST_HTMLDIR} ||= $self->catdir($Curdir,'blib','html');
...
sub init_INSTALL_from_PREFIX {
    ...
    (
	    htmldir       => { s => $iprefix,
			               t =>'perl',
			               d => 'html', },
	    sitehtmldir   => { s => $sprefix,
			               t => 'site',
			               d => 'html', },
	    vendorhtmldir => { s => $vprefix,
			               t => 'vendor',
			               d => 'html', },
    );
...
sub installvars {
    return qw(PRIVLIB SITELIB  VENDORLIB
              ...
              HTMLDIR SITEHTMLDIR VENDORHTMLDIR
             );
}
