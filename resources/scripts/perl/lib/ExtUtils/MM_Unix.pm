...
sub constants {
    ...
    for my $macro (qw(
              ...
              INST_MAN1DIR INST_MAN3DIR INST_HTMLDIR
              MAN1EXT      MAN3EXT
...
sub init_main {
    ...
    unless ( $Config{installsitehtmldir} ) {
        $Config_Override{installsitehtmldir} = $Config{installhtmldir};
    }

    unless ( $Config{installvendorhtmldir} ) {
        $Config_Override{installvendorhtmldir} = $Config{installhtmldir};
    }
...
sub install {
    ...
    if (eval { require ActivePerl::DocTools; 1; }) {
        $doc_update = '$(NOECHO) $(PERLRUN) "-MActivePerl::DocTools" -e '.
