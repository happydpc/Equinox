...
/**
 * Class for save stress sequence as STH process.
 *
 * @author Murat Artim
 * @date Jun 16, 2014
 * @time 8:55:32 AM
 */
public class SaveSTH implements EquinoxProcess<StressSequence> {
	...
	/**
	 * Creates save stress sequence as STH process.
	 *
	 * @param task
	 *            The owner task.
	 * @param sequence
	 ...
	 *            Output file.
	 */
	public SaveSTH(InternalEquinoxTask<?> task, StressSequence sequence, File output) {
		task_ = task;
		sequence_ = sequence;
		output_ = output;
	}
	...
	@Override
	public StressSequence start(Connection connection, PreparedStatement... preparedStatements) throws Exception {
	...
	private void writeFlights(Connection connection, BufferedWriter writer, int numFlights) throws Exception {
		...
		try (Statement selectFlights = connection.createStatement()) {
			...
			String sql = "select peak_val from sth_peaks_" + sequence_.getID() + " where flight_id = ? order by peak_num asc";
			try (PreparedStatement selectPeaks = connection.prepareStatement(sql)) {

				// get flights
				sql = "select flight_id, name, severity, num_peaks, validity, block_size from sth_flights where file_id = " + sequence_.getID() + " order by flight_num asc";
				try (ResultSet flights = selectFlights.executeQuery(sql)) {
					...
					while (flights.next()) {
						...
						line += (name.startsWith("TF_") ? name.substring(3) : name) + " " + flights.getString("severity");
						writer.write(line);
						writer.newLine();
	...
	private int writeHeader(Connection connection, BufferedWriter writer) throws Exception {
		...
		writer.newLine();
		writeCDFSetInfo(connection, writer);
	...
	private String getHighestLowest(Statement statement, String line, String label1, String label2, String colName, boolean isDesc) throws Exception {
		String sql = "select name, " + colName + " from sth_flights where file_id = " + sequence_.getID() + " order by " + colName + (isDesc ? " desc" : "");
	...
	private int writeSequenceInfo(Connection connection, BufferedWriter writer) throws Exception {
		...
		// create statement
		try (Statement statement = connection.createStatement()) {
			String sql = "select num_flights, oneg_fac, inc_fac, dp_fac, ref_dp, stress_comp, rotation_angle from sth_files where file_id = " + sequence_.getID();
			try (ResultSet resultSet = statement.executeQuery(sql)) {
				while (resultSet.next()) {
					String info = " STF file name: " + sequence_.getParentItem().getName() + ", ";
					numFlights = resultSet.getInt("num_flights");
					info += "Number of flights: " + Integer.toString(numFlights) + ", ";
					info += "Total number of peaks: " + Integer.toString(getTotalNumberOfPeaks(connection)) + ", ";
					info += "Reference DP: " + format_.format(resultSet.getDouble("ref_dp")) + ", ";
					info += "Stress component: " + resultSet.getString("stress_comp") + ", ";
					info += "Rotation angle: " + format_.format(Math.toDegrees(resultSet.getDouble("rotation_angle"))) + ", ";
					info += "1g stress modifier: " + resultSet.getString("oneg_fac") + ", ";
					info += "Increment stress modifier: " + resultSet.getString("inc_fac") + ", ";
					info += "Delta-p stress modifier: " + resultSet.getString("dp_fac") + ", ";
	...
	private int getTotalNumberOfPeaks(Connection connection) throws Exception {
		try (Statement statement = connection.createStatement()) {
			try (ResultSet resultSet = statement.executeQuery("select sum(num_peaks) as totalPeaks from sth_flights where file_id = " + sequence_.getID())) {
				if (resultSet.next())
	 ...
	 *             If exception occurs during process.
	 */
	private void writeCDFSetInfo(Connection connection, BufferedWriter writer) throws Exception {

		// create statement
		try (Statement statement = connection.createStatement()) {
			String sql = "select * from cdf_sets where set_id = " + sequence_.getParentItem().getParentItem().getID();
			try (ResultSet resultSet = statement.executeQuery(sql)) {
				while (resultSet.next()) {
					String info = " CDF set name: " + resultSet.getString("name") + ", ";
					info += "A/C program: " + resultSet.getString("ac_program") + ", ";
					info += "A/C section: " + resultSet.getString("ac_section") + ", ";
					info += "Fatigue mission: " + resultSet.getString("fat_mission") + ", ";
					info += "Fatigue mission issue: " + resultSet.getString("fat_mission_issue") + ", ";
					info += "FLP issue: " + resultSet.getString("flp_issue") + ", ";
					info += "IFLP issue: " + resultSet.getString("iflp_issue") + ", ";
					info += "CDF issue: " + resultSet.getString("cdf_issue");
					writer.write(info);
					writer.newLine();
				}
			}
