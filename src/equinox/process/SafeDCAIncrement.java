 ...
 * @time 12:26:27 PM
 */
public class SafeDCAIncrement implements Callable<DamageContributionResult>, AnalysisListenerTask {
	...
	private final String contributionName_;
	 ...
	 *            Omission level.
	 */
	public SafeDCAIncrement(LoadcaseDamageContributionAnalysis task, Path sthFile, Path flsFile, String contributionName, int contributionIndex, FatigueMaterial material, boolean applyOmission, double omissionLevel) {
		task_ = task;
		sthFile_ = sthFile;
		flsFile_ = flsFile;
	...
	public DamageContributionResult call() throws Exception {
		...
		try {
			...
			// process increment
			results = processIncrement(workingDir_, results);
	...
	private DamageContributionResult processIncrement(Path workingDir, DamageContributionResult results) throws Exception {
		...
		try {
			...
			// add input files
			task_.updateMessage("Adding input files for damage contribution " + contributionName_ + "...");
			...
			// zip input files
			task_.updateMessage("Zipping input files for damage contribution " + contributionName_ + "...");
			Path zipFile = workingDir.resolve("inputs_" + contributionName_ + ".zip");
			...
			task_.updateMessage("Uploading input files for damage contribution " + contributionName_ + "...");
			...
			// analysis succeeded
			else if (message instanceof DCAIncrementComplete) {
				...
				// extract results
				results.setDamage(completeMessage.getDamage());
				results.setStress(completeMessage.getStress());
				return results;
			}
	...
	private Path applyOmission(Path workingDir) throws Exception {
		...
		if (sthFileName == null)
			throw new Exception("Cannot get STH file name.");
