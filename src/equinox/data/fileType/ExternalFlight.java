...
/**
 * Class for external flight.
 *
 * @author Murat Artim
 * @date Mar 11, 2015
 * @time 10:03:54 AM
 */
public class ExternalFlight extends SpectrumItem {

	/**
	 * Creates external flight.
	 *
	 * @param flightName
	 *            Flight name.
	 * @param flightID
	 *            Flight ID.
	 */
	public ExternalFlight(String flightName, int flightID) {
	...
	@Override
	public ExternalFlights getParentItem() {
		return (ExternalFlights) getParent();
	}
	...
	public boolean equals(Object object) {
		if (object instanceof ExternalFlight) {
			ExternalFlight flight = (ExternalFlight) object;
	...
	public static ContextMenu createContextMenu(boolean multipleSelection, ActionHandler handler, ObservableList<TreeItem<String>> selected) {
		...
		if (multipleSelection) {
			...
			compare.setId("compareExternalFlights");
		...
		if (multipleSelection) {
			...
			for (TreeItem<String> item : selected) {
				ExternalFlight flight = (ExternalFlight) item;
				ExternalStressSequence seq = flight.getParentItem().getParentItem();
		...
		if (multipleSelection && fromSameSequence) {
			Label statisticsIcon = new Label("\uf080");
			statisticsIcon.getStylesheets().add(IconicFont.FONTAWESOME.getStyleSheet());
