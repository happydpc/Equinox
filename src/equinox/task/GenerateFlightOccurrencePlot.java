...
/**
 * Class for generate typical flight occurrence plot task.
 *
 * @author Murat Artim
 * @date 21 Jul 2016
 * @time 16:49:23
 */
public class GenerateFlightOccurrencePlot extends TemporaryFileCreatingTask<Path> implements LongRunningTask, SingleInputTask<SpectrumItem>, ParameterizedTaskOwner<Path> {
	 ...
	 */
	public GenerateFlightOccurrencePlot(SpectrumItem eqStress, boolean plot, Path output) {
		eqStress_ = eqStress;
		plot_ = plot;
		output_ = output;
	}
	...
	public String getTaskTitle() {
		return "Generate typical flight occurrence plot";
	}
	...
	protected Path call() throws Exception {
		...
		// update info
		updateMessage("Generating typical flight occurrence plot...");
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try {
				...
				try (Statement statement = connection.createStatement()) {
					...
					// plot typical flight occurrences
					Path file = plotOccurrences(statement);
	...
	protected void succeeded() {
		...
		// plot
		if (plot_) {
			taskPanel_.getOwner().runTaskInParallel(new PlotFastFlightOccurrences(eqStress_));
		}
	...
	/**
	 * Plots typical flight occurrences plot.
	 *
	 * @param statement
	 *            Database statement.
	 ...
	 */
	private Path plotOccurrences(Statement statement) throws Exception {

		// update info
		updateMessage("Plotting typical flight occurrences...");

		// create path to output image
		Path output = output_ == null ? getWorkingDirectory().resolve("typicalFlightOccurrences.png") : output_;

		// get STF file
		STFFile stfFile = (STFFile) eqStress_.getParentItem();

		// create chart
		String title = "Typical Flight Occurrences";
		title += "\n(" + FileType.getNameWithoutExtension(stfFile.getName()) + ")";
		JFreeChart chart = ChartFactory.createBarChart(title, "Flight", "Occurrence", null, PlotOrientation.VERTICAL, false, false, false);
	...
	private CategoryDataset generatePlot(Statement statement, STFFile stfFile) throws Exception {
		...
		// update info
		updateMessage("Generating typical flight occurrences plot...");
		...
		String sql = "select name, validity from ana_flights where ";
		sql += "file_id = " + anaFileID;
		sql += " order by validity desc";
		...
		try (ResultSet resultSet = statement.executeQuery(sql)) {
			...
			while (resultSet.next()) {
				...
				int peaks = resultSet.getInt("validity");
	...
	private void savePlot(Statement statement, Connection connection, Path file) throws Exception {
		...
		// update info
		updateMessage("Saving typical flight occurrences plot to database...");
		...
		// create statement
		if (exists) {
			sql = "update pilot_point_st_fo set image = ? where id = " + id;
		}
		else {
			sql = "insert into pilot_point_st_fo(id, image) values(?, ?)";
		}
