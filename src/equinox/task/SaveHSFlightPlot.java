...
/**
 * Class for save typical flight with highest total stress plot task.
 *
 * @author Murat Artim
 * @date 25 Jul 2016
 * @time 16:30:30
...
public class SaveHSFlightPlot extends TemporaryFileCreatingTask<Void> implements ShortRunningTask {
	...
	/**
	 * Creates save typical flight with highest total stress plot task.
	 *
	 * @param sequence
	 *            Stress sequence.
	 */
	public SaveHSFlightPlot(StressSequence sequence) {
		sequence_ = sequence;
	}
	...
	public String getTaskTitle() {
		return "Save typical flight with highest total stress plot for '" + sequence_.getName() + "'";
	}
	...
	protected Void call() throws Exception {
		...
		// update info
		updateMessage("Saving typical flight with highest total stress plot...");
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try (Statement statement = connection.createStatement()) {
				...
				Flight flight = getFlight(statement, "max_val", true);
	...
	private Path plotFlight(Statement statement, Flight flight) throws Exception {
		...
		// update info
		updateMessage("Plotting typical flight with highest total stress...");

		// create path to output image
		Path output = getWorkingDirectory().resolve("hsTypicalFlight.png");
