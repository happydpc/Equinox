...
/**
 * Class for get external fatigue equivalent stress info task.
 *
 * @author Murat Artim
 * @date Mar 15, 2015
 * @time 1:54:10 PM
 */
public class GetExternalFatigueEquivalentStressInfo extends InternalEquinoxTask<ArrayList<TreeItem<TableItem>>> implements ShortRunningTask {

	/** Fatigue equivalent stress item. */
	private final ExternalFatigueEquivalentStress equivalentStress_;

	/** Decimal format. */
	private final DecimalFormat format_ = new DecimalFormat("0.##");

	/**
	 * Creates get fatigue equivalent stress info task.
	 *
	 * @param equivalentStress
	 *            Equivalent stress item.
	 */
	public GetExternalFatigueEquivalentStressInfo(ExternalFatigueEquivalentStress equivalentStress) {
		equivalentStress_ = equivalentStress;
	}

	@Override
	public String getTaskTitle() {
		return "Get external fatigue equivalent stress info for '" + equivalentStress_.getName() + "'";
	}
	...
	protected ArrayList<TreeItem<TableItem>> call() throws Exception {
		...
		updateTitle("Retrieving external fatigue equivalent stress info '" + equivalentStress_.getName() + "'");
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try (Statement statement = connection.createStatement()) {
				...
				String sql = "select * from ext_fatigue_equivalent_stresses where id = " + equivalentStress_.getID();
				try (ResultSet resultSet = statement.executeQuery(sql)) {
					while (resultSet.next()) {
						...
						// stress modifier info
						list.add(new TreeItem<>(new TableItem("Overall stress modifier", resultSet.getString("overall_fac"))));

						// spectrum info
						TreeItem<TableItem> spectrum = new TreeItem<>(new TableItem("Spectrum info", ""));
						spectrum.getChildren().add(new TreeItem<>(new TableItem("A/C program", equivalentStress_.getParentItem().getProgram())));
						spectrum.getChildren().add(new TreeItem<>(new TableItem("A/C section", equivalentStress_.getParentItem().getSection())));
