 ...
 * @time 1:43:02 PM
 */
public class SaveStressSequenceAsSIGMA extends InternalEquinoxTask<Path> implements LongRunningTask, SingleInputTask<StressSequence>, ParameterizedTaskOwner<Path> {
	 ...
	 *            Output file.
	 */
	public SaveStressSequenceAsSIGMA(StressSequence sequence, File output) {
		sequence_ = sequence;
		output_ = output;
	}
	...
	public String getTaskTitle() {
		return "Save stress sequence to '" + output_.getName() + "'";
	}
	...
	protected Path call() throws Exception {
		...
		updateTitle("Saving stress sequence to '" + output_.getName() + "'");
	...
	private void writeFlights(Connection connection, Statement selectFlights, BufferedWriter writer) throws Exception {
		...
		try (PreparedStatement selectPeaks = connection.prepareStatement(sql)) {
			...
			// get flights
			sql = "select flight_id, flight_num, name, num_peaks, validity from sth_flights where file_id = " + sequence_.getID() + " order by flight_num asc";
	...
	private void writeFlightSequence(Connection connection, Statement statement, BufferedWriter writer) throws Exception {
		...
		String sql = "select flight_num from sth_flights where file_id = " + sequence_.getID() + " and name = ?";
		try (PreparedStatement statement2 = connection.prepareStatement(sql)) {

			// get flight names
			sql = "select name from fls_flights where file_id = " + sequence_.getParentItem().getParentItem().getFLSFileID() + " order by flight_num asc";
	...
	private void writeHeader(Statement statement, BufferedWriter writer) throws Exception {
		...
		String sql = "select sum(validity) as totalFlights from sth_flights where file_id = " + sequence_.getID();
		try (ResultSet resultSet = statement.executeQuery(sql)) {
			while (resultSet.next()) {
				String line = "NBVOL ";
		...
		sql = "select num_flights from sth_files where file_id = " + sequence_.getID();
		try (ResultSet resultSet = statement.executeQuery(sql)) {
			while (resultSet.next()) {
