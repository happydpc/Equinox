...
public class GenerateHOFlightPlot extends TemporaryFileCreatingTask<Path> implements LongRunningTask, SingleInputTask<SpectrumItem>, ParameterizedTaskOwner<Path> {
	 ...
	 */
	public GenerateHOFlightPlot(SpectrumItem eqStress, boolean plot, Path output) {
		eqStress_ = eqStress;
		plot_ = plot;
		output_ = output;
	}
	...
	private Path plotFlight(Connection connection, HOFlightInfo info, String peaksTableName, STFFile stfFile) throws Exception {
		...
		Path output = output_ == null ? getWorkingDirectory().resolve("highestOccurringTypicalFlight.png") : output_;
