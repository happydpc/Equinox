...
import equinox.task.serializableTask.SerializableSaveAircraftEquivalentStressRatios;
...
/**
 * Class for save A/C model equivalent stress ratios task.
 *
 * @author Murat Artim
 * @date Oct 2, 2015
 * @time 10:31:54 AM
 */
public class SaveAircraftEquivalentStressRatios extends InternalEquinoxTask<Void> implements LongRunningTask, SavableTask {

	/** Option index. */
	public static final int FAT_RATIO = 0, PROP_RATIO = 1, FAT_P = 2, ELBER_M = 3, EID = 4, MISSION = 5;
	...
	/**
	 * Creates save A/C model equivalent stress ratios task.
	 *
	 * @param eqStress
	 *            Equivalent stress.
	 * @param options
	 ...
	 *            Basis mission.
	 */
	public SaveAircraftEquivalentStressRatios(AircraftFatigueEquivalentStress eqStress, BooleanProperty[] options, File output, String basisMission) {
		eqStress_ = eqStress;
		options_ = options;
		output_ = output;
	...
	public String getTaskTitle() {
		return "Save aircraft equivalent stress ratios";
	}

	@Override
	public SerializableTask getSerializableTask() {
		return new SerializableSaveAircraftEquivalentStressRatios(eqStress_, options_, output_, basisMission_);
	}
	...
	protected Void call() throws Exception {
		...
		updateTitle("Saving aircraft equivalent stress ratios to '" + output_.getName() + "'");
		...
		try {
			...
			// create worksheet
			WritableSheet sheet = workbook.createSheet("Equivalent Stress Ratios", 0);
			...
			try (Statement statement2 = connection.createStatement()) {
				try (ResultSet getBasisMission = statement2.executeQuery(createBasisMissionQuery())) {
					...
					while (getBasisMission.next()) {
						...
						statement1.setDouble(2, getBasisMission.getDouble("prop_stress"));
						...
						try (ResultSet getOtherMission = statement1.executeQuery()) {
							...
							while (getOtherMission.next()) {
								...
								double fatP = getOtherMission.getDouble("fat_p");
								double elberM = getOtherMission.getDouble("elber_m");
								double fatRatio = getOtherMission.getDouble("fatRatio");
								double propRatio = getOtherMission.getDouble("propRatio");

								// write data row
								writeDataRow(sheet, eid, mission, fatRatio, propRatio, fatP, elberM, row);
								row++;
							}
						}
					}
	 ...
	 *             If exception occurs during process.
	 */
	private void writeDataRow(WritableSheet sheet, int eid, String mission, double fatRatio, double propRatio, double fatP, double elberM, int row) throws Exception {
		...
		// fatigue ratio
		if (options_[FAT_RATIO].get()) {
			sheet.addCell(new jxl.write.Number(column, row, fatRatio, getDataFormat(row, CellType.NUMBER)));
			column++;
		}

		// propagation ratio
		if (options_[PROP_RATIO].get()) {
			sheet.addCell(new jxl.write.Number(column, row, propRatio, getDataFormat(row, CellType.NUMBER)));
			column++;
		}
	}
	...
	private String createOtherMissionQuery() throws Exception {
		...
		// select stresses
		String sql = "select mission, fat_p, elber_m, ";
		sql += "fat_stress/? as fatRatio, ";
		sql += "prop_stress/? as propRatio";
		sql += " from " + stressTable;
	...
	private void writeHeaders(WritableSheet sheet) throws Exception {
		...
		// fatigue ratio
		if (options_[FAT_RATIO].get()) {
			String header = ((ToggleSwitch) options_[FAT_RATIO].getBean()).getText();
			sheet.addCell(new jxl.write.Label(column, 0, header, format));
			sheet.setColumnView(column, header.length());
			column++;
		}
		...
		// propagation ratio
		if (options_[PROP_RATIO].get()) {
			String header = ((ToggleSwitch) options_[PROP_RATIO].getBean()).getText();
			sheet.addCell(new jxl.write.Label(column, 0, header, format));
			sheet.setColumnView(column, header.length());
			column++;
		}
