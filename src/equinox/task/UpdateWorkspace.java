...
public class UpdateWorkspace extends InternalEquinoxTask<Void> {
	...
	private double updateTo40(Connection connection) throws Exception {
		...
		try (Statement statement = connection.createStatement()) {
			...
			statement.executeUpdate("CREATE TABLE AURORA.SAVED_INSTRUCTION_SETS(ID INT NOT NULL GENERATED ALWAYS AS IDENTITY(START WITH 1, INCREMENT BY 1), SCHEDULE_DATE TIMESTAMP, FILE_NAME VARCHAR(500) NOT NULL, DATA BLOB(30M) NOT NULL, PRIMARY KEY(ID))");
			statement.executeUpdate("CREATE INDEX INSTRUCTION_SET_ID ON AURORA.SAVED_INSTRUCTION_SETS(ID)");
		}
