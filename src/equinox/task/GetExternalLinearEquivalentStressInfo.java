...
/**
 * Class for getting external linear equivalent stress info.
 *
 * @author Murat Artim
 * @date Jul 7, 2014
 * @time 2:57:21 PM
 */
public class GetExternalLinearEquivalentStressInfo extends InternalEquinoxTask<ArrayList<TreeItem<TableItem>>> implements ShortRunningTask {

	/** Linear equivalent stress item. */
	private final ExternalLinearEquivalentStress equivalentStress_;

	/** Decimal format. */
	private final DecimalFormat format_ = new DecimalFormat("0.##"), format2_ = new DecimalFormat("0.##E0");

	/**
	 * Creates get external linear equivalent stress info task.
	 *
	 * @param equivalentStress
	 *            Linear equivalent stress item.
	 */
	public GetExternalLinearEquivalentStressInfo(ExternalLinearEquivalentStress equivalentStress) {
		equivalentStress_ = equivalentStress;
	}

	@Override
	public String getTaskTitle() {
		return "Get external linear equivalent stress info for '" + equivalentStress_.getName() + "'";
	}
	...
	protected ArrayList<TreeItem<TableItem>> call() throws Exception {
		...
		updateTitle("Retrieving external linear equivalent stress info '" + equivalentStress_.getName() + "'");
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try (Statement statement = connection.createStatement()) {
				...
				String sql = "select * from ext_linear_equivalent_stresses where id = " + equivalentStress_.getID();
