...
/**
 * Class for save highest occurring typical flight plot task.
 *
 * @author Murat Artim
 * @date 25 Jul 2016
 * @time 16:24:34
 */
public class SaveHOFlightPlot extends TemporaryFileCreatingTask<Void> implements ShortRunningTask, PostProcessingTask {

	/** Stress sequence. */
	private final StressSequence sequence_;

	/**
	 * Creates save highest occurring typical flight plot task.
	 *
	 * @param sequence
	 *            Stress sequence.
	 */
	public SaveHOFlightPlot(StressSequence sequence) {
		sequence_ = sequence;
	}
	...
	public String getTaskTitle() {
		return "Save highest occurring typical flight for '" + sequence_.getName() + "'";
	}
	...
	protected Void call() throws Exception {
		...
		// update info
		updateMessage("Saving highest occurring typical flight plot...");
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try (Statement statement = connection.createStatement()) {
				...
				// get typical flight info
				Flight flight = getFlight(statement, "validity", true);
	...
	private Path plotFlight(Statement statement, Flight flight) throws Exception {
		...
		// update info
		updateMessage("Plotting highest occurring typical flight...");

		// create path to output image
		Path output = getWorkingDirectory().resolve("hoTypicalFlight.png");
