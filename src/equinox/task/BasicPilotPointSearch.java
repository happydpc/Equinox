...
import equinoxServer.remote.message.BasicPilotPointSearchRequest;
import equinoxServer.remote.message.BasicPilotPointSearchResponse;
...
public class BasicPilotPointSearch extends InternalEquinoxTask<ArrayList<DownloadInfo>> implements ShortRunningTask, DatabaseQueryListenerTask {
	 ...
	 *            Search input.
	 */
	public BasicPilotPointSearch(BasicSearchInput input) {
		this.input = input;
	...
	protected ArrayList<DownloadInfo> call() throws Exception {
		...
		try {
			...
			BasicPilotPointSearchRequest request = new BasicPilotPointSearchRequest();
			...
			// query succeeded
			else if (message instanceof BasicPilotPointSearchResponse) {
				results = ((BasicPilotPointSearchResponse) message).getSearchResults();
			}
