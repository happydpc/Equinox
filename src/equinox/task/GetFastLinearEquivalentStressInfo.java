...
/**
 * Class for getting fast linear equivalent stress info.
 *
 * @author Murat Artim
 * @date Jun 15, 2016
 * @time 1:08:09 PM
 */
public class GetFastLinearEquivalentStressInfo extends InternalEquinoxTask<ArrayList<TreeItem<TableItem>>> implements ShortRunningTask {

	/** Linear equivalent stress item. */
	private final FastLinearEquivalentStress equivalentStress_;

	/** Decimal format. */
	private final DecimalFormat format_ = new DecimalFormat("0.##"), format2_ = new DecimalFormat("0.##E0");

	/**
	 * Creates get fast linear equivalent stress info task.
	 *
	 * @param equivalentStress
	 *            Linear equivalent stress item.
	 */
	public GetFastLinearEquivalentStressInfo(FastLinearEquivalentStress equivalentStress) {
	...
	protected ArrayList<TreeItem<TableItem>> call() throws Exception {
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try (Statement statement = connection.createStatement()) {
				...
				String sql = "select * from fast_linear_equivalent_stresses where id = " + equivalentStress_.getID();
