...
/**
 * Class for getting external preffas equivalent stress info.
 *
 * @author Murat Artim
 * @date Jul 7, 2014
 * @time 2:57:21 PM
 */
public class GetExternalPreffasEquivalentStressInfo extends InternalEquinoxTask<ArrayList<TreeItem<TableItem>>> implements ShortRunningTask {

	/** Preffas equivalent stress item. */
	private final ExternalPreffasEquivalentStress equivalentStress_;

	/** Decimal format. */
	private final DecimalFormat format_ = new DecimalFormat("0.##"), format2_ = new DecimalFormat("0.##E0");

	/**
	 * Creates get external preffas equivalent stress info task.
	 *
	 * @param equivalentStress
	 *            Preffas equivalent stress item.
	 */
	public GetExternalPreffasEquivalentStressInfo(ExternalPreffasEquivalentStress equivalentStress) {
		equivalentStress_ = equivalentStress;
	}

	@Override
	public String getTaskTitle() {
		return "Get external preffas equivalent stress info for '" + equivalentStress_.getName() + "'";
	}
	...
	protected ArrayList<TreeItem<TableItem>> call() throws Exception {
		...
		updateTitle("Retrieving external preffas equivalent stress info '" + equivalentStress_.getName() + "'");
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try (Statement statement = connection.createStatement()) {
				...
				String sql = "select * from ext_preffas_equivalent_stresses where id = " + equivalentStress_.getID();
