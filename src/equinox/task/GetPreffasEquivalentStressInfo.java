...
/**
 * Class for getting preffas equivalent stress info.
 *
 * @author Murat Artim
 * @date Jul 7, 2014
 * @time 2:57:21 PM
 */
public class GetPreffasEquivalentStressInfo extends InternalEquinoxTask<ArrayList<TreeItem<TableItem>>> implements ShortRunningTask {

	/** Preffas equivalent stress item. */
	private final PreffasEquivalentStress equivalentStress_;

	/** Decimal format. */
	private final DecimalFormat format_ = new DecimalFormat("0.##"), format2_ = new DecimalFormat("0.##E0");

	/**
	 * Creates get preffas equivalent stress info task.
	 *
	 * @param equivalentStress
	 *            Preffas equivalent stress item.
	 */
	public GetPreffasEquivalentStressInfo(PreffasEquivalentStress equivalentStress) {
	...
	protected ArrayList<TreeItem<TableItem>> call() throws Exception {
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try (Statement statement = connection.createStatement()) {
				...
				String sql = "select * from preffas_equivalent_stresses where id = " + equivalentStress_.getID();
