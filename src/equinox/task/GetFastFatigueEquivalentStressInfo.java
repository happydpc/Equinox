...
/**
 * Class for get fast fatigue equivalent stress info task.
 *
 * @author Murat Artim
 * @date Jun 15, 2016
 * @time 12:54:25 PM
 */
public class GetFastFatigueEquivalentStressInfo extends InternalEquinoxTask<ArrayList<TreeItem<TableItem>>> implements ShortRunningTask {

	/** Fatigue equivalent stress item. */
	private final FastFatigueEquivalentStress equivalentStress_;

	/** Decimal format. */
	private final DecimalFormat format_ = new DecimalFormat("0.#####");

	/**
	 * Creates get fast fatigue equivalent stress info task.
	 *
	 * @param equivalentStress
	 *            Fatigue equivalent stress item.
	 */
	public GetFastFatigueEquivalentStressInfo(FastFatigueEquivalentStress equivalentStress) {
		equivalentStress_ = equivalentStress;
	}

	@Override
	public String getTaskTitle() {
		return "Get fatigue equivalent stress info for '" + equivalentStress_.getName() + "'";
	}
	...
	protected ArrayList<TreeItem<TableItem>> call() throws Exception {
		...
		updateTitle("Retrieving fatigue equivalent stress info '" + equivalentStress_.getName() + "'");
		...
		try (Connection connection = Equinox.DBC_POOL.getConnection()) {
			...
			try (Statement statement = connection.createStatement()) {
				...
				String sql = "select * from fast_fatigue_equivalent_stresses where id = " + equivalentStress_.getID();
