...
/**
 * Class for external level crossing panel controller.
 *
 * @author Murat Artim
 * @date Mar 15, 2015
 * @time 3:57:55 PM
 */
public class ExternalLevelCrossingPanel implements InternalInputSubPanel {

	/** The owner panel. */
	private InputPanel owner_;

	/** Plot input. */
	private ExternalLevelCrossingInput input_ = null;

	@FXML
	private VBox root_, dsgContainer_, seriesContainer_;

	@FXML
	private ToggleSwitch normalize_, includeSequenceName_, includeEID_, includeMaterialName_, includeOmissionLevel_, includeProgram_, includeSection_, includeMission_, showInfo_, showMarkers_, showCrosshair_;
	...
	public void showing() {
		...
		for (TreeItem<String> item : selected) {
			if (item instanceof ExternalFatigueEquivalentStress) {
				addFatigueEquivalentStress((ExternalFatigueEquivalentStress) item);
			}
			else if (item instanceof ExternalPreffasEquivalentStress) {
				addPreffasEquivalentStress((ExternalPreffasEquivalentStress) item);
			}
			else if (item instanceof ExternalLinearEquivalentStress) {
				addLinearEquivalentStress((ExternalLinearEquivalentStress) item);
			}
		}
	...
	private boolean checkInputs() {
		...
		// no naming selected
		if (!includeSequenceName && !includeEID && !includeMaterialName && !includeOmissionLevel && !includeProgram && !includeSection && !includeMission) {
	...
	/**
	 * Called to setup the refresh button.
	 *
	 */
	 ...
	 */
	private void addFatigueEquivalentStress(ExternalFatigueEquivalentStress eqStress) {
	 ...
	 */
	private void addPreffasEquivalentStress(ExternalPreffasEquivalentStress eqStress) {
		IntegerValidationField tf = new IntegerValidationField();
	 ...
	 */
	private void addLinearEquivalentStress(ExternalLinearEquivalentStress eqStress) {
		IntegerValidationField tf = new IntegerValidationField();
	 ...
	 */
	private static String getFatigueEquivalentStressName(ExternalFatigueEquivalentStress eqStress) {
		ExternalStressSequence sequence = eqStress.getParentItem();
		String name = sequence.getProgram();
	 ...
	 */
	private static String getPreffasEquivalentStressName(ExternalPreffasEquivalentStress eqStress) {
		ExternalStressSequence sequence = eqStress.getParentItem();
		String name = sequence.getProgram();
		name += ", " + sequence.getSection();
		name += ", " + sequence.getMission();
	 ...
	 */
	private static String getLinearEquivalentStressName(ExternalLinearEquivalentStress eqStress) {
		ExternalStressSequence sequence = eqStress.getParentItem();
		String name = sequence.getProgram();
		name += ", " + sequence.getSection();
		name += ", " + sequence.getMission();
	 ...
	 */
	public static ExternalLevelCrossingPanel load(InputPanel owner) {

		try {

			// load fxml file
			FXMLLoader fxmlLoader = new FXMLLoader(EquinoxTheme.getFXMLResource("ExternalLevelCrossingPanel.fxml"));
			...
			// get controller
			ExternalLevelCrossingPanel controller = (ExternalLevelCrossingPanel) fxmlLoader.getController();
