...
import equinox.data.input.PlotAircraftEquivalentStressRatiosInput;
...
/**
 * Class for plot A/C model equivalent stress ratios panel controller.
 *
 * @author Murat Artim
 * @date Oct 4, 2015
 * @time 6:44:03 PM
 */
public class PlotAircraftEquivalentStressRatiosPanel implements InternalInputSubPanel, ElementGroupsRequestingPanel, MissionRequestingPanel {
	...
	@FXML
	private ComboBox<EquivalentStressRatioType> ratioType_;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		ratioType_.setItems(FXCollections.observableArrayList(EquivalentStressRatioType.values()));
	...
	private void onOkClicked() {
		...
		EquivalentStressRatioType ratioType = ratioType_.getSelectionModel().getSelectedItem();
		...
		// create input
		PlotAircraftEquivalentStressRatiosInput input = new PlotAircraftEquivalentStressRatiosInput(selected, elementType, ratioType, mission, basisMission, beamExtrusion, rodExtrusion, opacity, outlines, contour);
		...
		// owner_.getOwner().getActiveTasksPanel().runTaskSequentially(new PlotAircraftEquivalentStressRatios(input));
	}
	...
	private boolean checkInputs(AircraftFatigueEquivalentStress selected) {
		...
		// no equivalent stress ratio type selected
		EquivalentStressRatioType ratioType = ratioType_.getSelectionModel().getSelectedItem();
		if (ratioType == null) {
			String message = "Please select ratio type to proceed.";
			PopOver popOver = new PopOver();
			popOver.setArrowLocation(ArrowLocation.TOP_LEFT);
			popOver.setDetachable(false);
			popOver.setContentNode(NotificationPanel1.load(message, 30, NotificationPanel1.WARNING));
	...
	private void onResetClicked() {
		...
		ratioType_.getSelectionModel().clearSelection();
		mission_.getSelectionModel().clearSelection();
		basisMission_.getSelectionModel().clearSelection();
	 ...
	 */
	public static PlotAircraftEquivalentStressRatiosPanel load(InputPanel owner) {

		try {

			// load fxml file
			FXMLLoader fxmlLoader = new FXMLLoader(EquinoxTheme.getFXMLResource("PlotAircraftEquivalentStressRatiosPanel.fxml"));
			...
			// get controller
			PlotAircraftEquivalentStressRatiosPanel controller = (PlotAircraftEquivalentStressRatiosPanel) fxmlLoader.getController();
