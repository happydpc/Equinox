...
import equinox.task.SaveAircraftEquivalentStressRatios;
...
/**
 * Class for save A/C model equivalent stress ratio panel controller.
 *
 * @author Murat Artim
 * @date Oct 1, 2015
 * @time 4:25:21 PM
 */
public class SaveAircraftEquivalentStressRatioPanel implements InternalInputSubPanel, ListChangeListener<ExchangeUser>, MissionRequestingPanel, SchedulingPanel {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		for (BooleanProperty option : options_) {
			if (option.equals(options_[SaveAircraftEquivalentStressRatios.FAT_RATIO])) {
				option.set(true);
			}
			else if (option.equals(options_[SaveAircraftEquivalentStressRatios.PROP_RATIO])) {
				option.set(true);
			}
			else if (option.equals(options_[SaveAircraftEquivalentStressRatios.EID])) {
				option.set(true);
			}
			else if (option.equals(options_[SaveAircraftEquivalentStressRatios.MISSION])) {
				option.set(true);
			}
			else {
				option.set(false);
	...
	public String getHeader() {
		return isSave_ ? "Save Eq. Stress Ratios" : "Share Eq. Stress Ratios";
	}
	...
	/**
	 * Saves equivalent stress ratios.
	 *
	 * @param runNow
	...
	private void save(boolean runNow, Date scheduleDate) {
		...
		// run now
		if (runNow) {
			tm.runTaskInParallel(new SaveAircraftEquivalentStressRatios(eqStress, options_, output, basisMission));
		}
		else {
			tm.runTaskInParallel(new SaveTask(new SaveAircraftEquivalentStressRatios(eqStress, options_, output, basisMission), scheduleDate));
		}
	...
	/**
	 * Shares equivalent stress ratios.
	 *
	 * @param runNow
	 *            True if task(s) should be run right now.
	 * @param scheduleDate
	...
	private void share(boolean runNow, Date scheduleDate) {
		...
		Path workingDirectory = createWorkingDirectory("ShareAircraftEquivalentStressRatio");
		...
		// create tasks
		SaveAircraftEquivalentStressRatios saveTask = new SaveAircraftEquivalentStressRatios(eqStress, options_, output.toFile(), basisMission);
	...
	private void onHelpClicked() {
		owner_.getOwner().showHelp("How to save aircraft equivalent stress ratios", null);
	}
	 ...
	 */
	public static SaveAircraftEquivalentStressRatioPanel load(InputPanel owner) {

		try {

			// load fxml file
			FXMLLoader fxmlLoader = new FXMLLoader(EquinoxTheme.getFXMLResource("SaveAircraftEquivalentStressRatioPanel.fxml"));
			...
			// get controller
			SaveAircraftEquivalentStressRatioPanel controller = (SaveAircraftEquivalentStressRatioPanel) fxmlLoader.getController();
