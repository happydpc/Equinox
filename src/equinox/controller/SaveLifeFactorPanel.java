...
/**
 * Class for save life factor panel controller.
 *
 * @author Murat Artim
 * @date Sep 28, 2015
 * @time 2:07:43 PM
 */
public class SaveLifeFactorPanel implements InternalInputSubPanel, ListChangeListener<ExchangeUser>, SchedulingPanel {
	...
	@FXML
	private ToggleSwitch lifeFactor_, materialName_, materialData_, ppName_, eid_, sequenceName_, spectrumName_, program_, section_, mission_, validity_, maxStress_, minStress_, rRatio_, omission_;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		// setup options
		options_ = new BooleanProperty[] { lifeFactor_.selectedProperty(), materialName_.selectedProperty(), materialData_.selectedProperty(), ppName_.selectedProperty(), eid_.selectedProperty(), sequenceName_.selectedProperty(), spectrumName_.selectedProperty(), program_.selectedProperty(),
		...
		options_[SaveLifeFactors.LIFE_FACTOR].set(true);
		options_[SaveLifeFactors.PP_NAME].set(true);
		options_[SaveLifeFactors.MISSION].set(true);
	...
	private void share(boolean runNow, Date scheduleDate) {
		...
		Path workingDirectory = createWorkingDirectory("ShareLifeFactor");
		...
		// create tasks
		SaveLifeFactors saveTask = new SaveLifeFactors(stresses, options_, output.toFile(), basisMission);
	...
	private void save(boolean runNow, Date scheduleDate) {
		...
		// run now
		if (runNow) {
			tm.runTaskInParallel(new SaveLifeFactors(stresses, options_, output, basisMission));
		}
		else {
			tm.runTaskInParallel(new SaveTask(new SaveLifeFactors(stresses, options_, output, basisMission), scheduleDate));
		}
	...
	private void onResetClicked() {
		...
		for (int i = 0; i < options_.length; i++) {
			if (i == SaveLifeFactors.LIFE_FACTOR) {
				if (!options_[i].get()) {
					options_[i].set(true);
				}
			}
			else if (i == SaveLifeFactors.MISSION) {
				if (!options_[i].get()) {
					options_[i].set(true);
				}
			}
	...
	private void onHelpClicked() {
		owner_.getOwner().showHelp("How to save life factors", null);
	}
	 ...
	 */
	public static SaveLifeFactorPanel load(InputPanel owner) {

		try {

			// load fxml file
			FXMLLoader fxmlLoader = new FXMLLoader(EquinoxTheme.getFXMLResource("SaveLifeFactorPanel.fxml"));
			...
			// get controller
			SaveLifeFactorPanel controller = (SaveLifeFactorPanel) fxmlLoader.getController();
