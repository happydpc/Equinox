...
import equinox.task.PlotSpectrumSize;
...
/**
 * Class for data insights spectrum size panel controller.
 *
 * @author Murat Artim
 * @date 27 Jul 2017
 * @time 12:44:57
 *
 */
public class DataInsightsSpectrumSizePanel implements DataInsightsSubPanel, SpectrumAircraftProgramRequestingPanel, SpectrumAircraftSectionRequestingPanel, SpectrumFatigueMissionRequestingPanel {
	...
	public DataInsightType getType() {
		return DataInsightType.SPECTRUM_SIZE;
	}
	...
	private void onOkClicked() {
		...
		// plot
		owner_.getOwner().getOwner().getActiveTasksPanel().runTaskInParallel(new PlotSpectrumSize(program, section, mission));
	}
	 ...
	 */
	public static DataInsightsSpectrumSizePanel load(DataInsightsPanel owner) {

		try {

			// load fxml file
			final FXMLLoader fxmlLoader = new FXMLLoader(EquinoxTheme.getFXMLResource("DataInsightsSpectrumSizePanel.fxml"));
			...
			// get controller
			final DataInsightsSpectrumSizePanel controller = (DataInsightsSpectrumSizePanel) fxmlLoader.getController();
